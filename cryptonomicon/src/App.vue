<template>
  ...
  <!-- <p>HelloWorld {{ ticker }}</p>
  <input type="text" v-model="ticker" @keydown.enter="add()" />
  <button @click="add()">Add</button>
  <div class="conteiner">
    <div class="tickerbox" v-for="t in tikers" :key="t">
      <h1>{{ t.name }}</h1>
      <h2>{{ t.price }}</h2>
      <button @click="deleteBox(t)">Delete</button>
    </div>
  </div> -->
</template>
...
export default {
  ...
  methods: {
    add() {
      ...
      this.tikers.push(newTicker);
      this.ticker = "";
    },

    deleteBox(tToDelete) {
      this.tikers = this.tickers.filter((t) => t != tToDelete);
    },
  },
};
</script>
